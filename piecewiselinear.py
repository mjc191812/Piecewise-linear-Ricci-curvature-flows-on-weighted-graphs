...
def main():
    ...
    G = nx.read_gexf(r"C:\Users\20978\Desktop\data\karate.gexf")
    maxw=max(nx.get_edge_attributes(G, "weight").values())
    minw=min(nx.get_edge_attributes(G, "weight").values())
    A=2*maxw/minw
    c_i=False
    ricci_G = RhoCurvature(G)
    forman_G = RhoCurvature(G)
    menger_G = RhoCurvature(G)
    haantjes_G = RhoCurvature(G)
    LLY_G = RhoCurvature(G,alpha=0)
    ricci_G._get_all_pairs_shortest_path()
    ricci_G.compute_ricci_curvature()
    forman_G.lengths=ricci_G.lengths
    forman_G.compute_forman_ricci_curvatures()
    menger_G.lengths=ricci_G.lengths    
    menger_G.compute_Menger_ricci_curvatures()
    haantjes_G.lengths=ricci_G.lengths
    haantjes_G.compute_haantjes_curvature()
    LLY_G.lengths=ricci_G.lengths
    LLY_G.compute_Lin_Lu_yau_ricci_curvature()
    ...
    def somthing(max_iter,time_interval,ricci_G,curvature="ricciCurvature"):
        start_time = time.time()
        ricci_G.length=ricci_G._get_all_pairs_shortest_path()
        if curvature == "ricciCurvature":           
            ricci_G.compute_ricci_curvature()
        elif curvature == "forman_ricci_curvature":
            ricci_G.compute_forman_ricci_curvatures()
        elif curvature == "Menger_ricci_curvature":
           
            ricci_G.compute_Menger_ricci_curvatures()
        elif curvature == "haantjes_curvature":
            
            ricci_G.compute_haantjes_curvature()
        elif curvature == "LLY_curvature":           
            ricci_G.compute_Lin_Lu_yau_ricci_curvature()
        ...
        for iter in range(1,max_iter):
            ...
            for n1, n2 in ricci_G.G.edges():
                ...
                result=safe_exponential(-(t-last_t) * ricci_G.G[n1][n2][curvature])
        ...
        a,b,c=check_accuracy(ricci_G.G, weight="weight", clustering_label="club")
        

        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"time: {elapsed_time} s")
        #reture ari or nmi or modularity
        return b
    

    
    #Some hyperparameters that work well

    #somthing(141,10,ricci_G,curvature="ricciCurvature")
    #somthing(14,1,forman_G,curvature='forman_ricci_curvature')
    #somthing(32,10,menger_G,curvature='Menger_ricci_curvature')
    #somthing(1,10,haantjes_G,curvature='haantjes_curvature')
    #somthing(2,33,LLY_G,curvature='LLY_curvature')
    ...
    def objective(trial):
        ...
        time_interval = trial.suggest_int('time_interval', 1, 100)
        new_G = deepcopy(ricci_G)
        loss=somthing(max_iter,time_interval,new_G,curvature="ricciCurvature")
        return loss
    
    study = optuna.create_study(direction='maximize')
    study.optimize(objective, n_trials=100) 
    print(f"bset value: {study.best_value}") 
